...
void TriMesh::drawLine(Vector3d s, Vector3d e){
	glColor4f(0, 0, 1, 1);
	glBegin(GL_LINES);
...
void TriMesh::drawVertex(vector<Vector3d> &vertices, Vector4d vertColor){
...
void TriMesh::draw(Vector4d fillColor, Vector4d edgeColor) {
    ...
    Vector4d vertColor;
    vertColor << 0, 0, 0, 1;
    //drawVertex(vertices, vertColor);	// draw vertices for debugging
}
...
TriMesh TriMesh::box(Vector3d center, Vector3d axis1, Vector3d axis2, Vector3d axis3) {
    TriMesh mesh;
    for (int i = -1; i <= 1; i += 2)
        for (int j = -1; j <= 1; j += 2)
            for (int k = -1; k <= 1; k += 2)
    ...
    // Add edge nodes
    int numEdgeNode = 0; // number of edge nodes
    Vector3d ref1;
    Vector3d ref2;
    Vector3d var;
    for(int i=0; i<3; i++){
    	...
    	for(int i=1; i<=numEdgeNode; i++){
    		double frac = ((double)i)/(1.0+(double)numEdgeNode);
			mesh.addVertex( center - ref1 - ref2 - var + frac*2.*var );
			mesh.addVertex( center - ref1 + ref2 - var + frac*2.*var );
			mesh.addVertex( center + ref1 - ref2 - var + frac*2.*var );
			mesh.addVertex( center + ref1 + ref2 - var + frac*2.*var );
    	}
...
TriMesh TriMesh::card(Vector3d center, Vector3d axis1, Vector3d axis2, Vector3d axis3){ // width, height, thickness
    TriMesh mesh;
    for (int i = -1; i <= 1; i += 2)
        for (int j = -1; j <= 1; j += 2)
            for (int k = -1; k <= 1; k += 2)
    ...
    // Add edge nodes
    for(int i=0; i<2; i++){	// add height node and add width node
    	Vector3d ref1;
    	Vector3d ref2;
    	Vector3d var;
    	switch (i){
    		case 0:
    			ref1 = axis3/2; // thickness
    			ref2 = axis2/2;
    			var = axis1/2;  			
    			break;
    		case 1:
    	...
    	int numEdgeNode = 2; //(int)(0.25*var.norm()/ref1.norm()); // number of edge nodes
